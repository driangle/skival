...
func buildStepVerifier(step suite.VerifyStep, i int, dirs stepDirs, cfg pipelineConfig) (namedVerifier, bool) {
	...
	case "check":
		...
		return named(step.Name, "check", &CheckVerifier{Dir: dirs.work, Command: cmd}), true
	case "check_output":
		cmd := expandPathVars(step.Run, dirs)
		return named(step.Name, "check_output", &CheckOutputVerifier{Command: cmd, Dir: dirs.work}), true
