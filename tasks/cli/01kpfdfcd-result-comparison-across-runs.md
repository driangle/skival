...
- [ ] Create `internal/compare/` package with a `Compare(baseline, candidate SuiteResult)` function that computes per-treatment deltas
- [ ] Compute deltas for: pass rate (percentage points), median cost (USD and %), median duration (ms and %)
- [ ] Handle mismatched evals/treatments gracefully (report added/removed treatments, skip unmatched)
- [ ] Add markdown comparison report output with delta columns and directional indicators (↑/↓/=)
- [ ] Add JSON comparison report output for programmatic consumption
- [ ] Add `skival compare <baseline-dir> <candidate-dir>` Cobra command with `--format` flag
- [ ] Add tests for comparison logic (matching treatments, mismatched treatments, identical results)
- [ ] Add tests for the CLI command
- [ ] Update CLI documentation (cli.md) with the new command
