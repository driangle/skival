...
- [ ] Add a version constant to the Go CLI (e.g. `apps/cli/cmd/root.go`) that can be set via `-ldflags` at build time
- [ ] Create `.release.conf` listing version files for the `/release` skill
- [ ] Create `.github/workflows/release.yml` triggered on `v*` tags that:
...
- [ ] Add a Homebrew formula update step to the release workflow that pushes to `driangle/homebrew-tap`
- [ ] Verify end-to-end: dry-run `/release` and inspect the generated workflow artifacts
