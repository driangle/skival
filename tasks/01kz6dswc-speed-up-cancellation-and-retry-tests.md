...
- [x] Rework verifier cancellation tests to cancel immediately and assert prompt
      return, instead of racing a long real sleep
- [x] Inject a clock / make backoff delays overridable so `retry` tests don't
      sleep real seconds (`internal/executor/retry.go`, `runSample` backoff)
- [x] Ensure any remaining real-time waits use short, bounded deadlines
- [x] Confirm total `go test ./...` wall-clock drops substantially
